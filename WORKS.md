...
- **商業出版社から商業出版として刊行された作品は、原則としてMoonnet Sagaの本文収録対象にしません。**
- そのような作品はオープンライセンス対象にも含めません。
- 必要に応じて書誌・世界観上の参照を行うことはできますが、本文をこのリポジトリへコピーする運用は行いません。
- この方針は権利上公開可能かどうかとは別の、Moonnet Saga側の編集・運用方針です。
...
- Saga status が `confirmed` または `possible` で、公開に必要な権利・版を確認できた作品は、このリポジトリの `works/` にコピーする。ただし、商業出版作品は上記の収録対象方針を優先する。
