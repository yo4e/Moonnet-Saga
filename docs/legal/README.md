# Legal / Rights Research

Moonnet Saga の公開・権利・将来のオープンライセンス化に関する調査資料。

> ここにある文書は法的助言ではなく、実務上の確認事項を整理した調査メモです。

## Documents

- [`legal-risk-audit.md`](legal-risk-audit.md) — 現状の法的・権利リスク監査
- [`works-rights-matrix.md`](works-rights-matrix.md) — 作品別の権利確認台帳
- [`open-license-checklist.md`](open-license-checklist.md) — CC BY-SA 4.0等を実際に発効する前のチェックリスト
- [`author-confirmations.md`](author-confirmations.md) — 作者・編集部から得た日付付きの権利確認事項とプロジェクト方針

`author-confirmations.md` に後日追加された事実確認は、初回監査時点で `要確認` としていた事項を更新するものとして扱います。

## Current rule

現時点では Moonnet Saga は **All Rights Reserved**。

CC BY-SA 4.0等は将来構想であり、このディレクトリの追加によってライセンスは変更されない。

また、商業出版社から商業出版として刊行された作品は、原則としてMoonnet Sagaの本文収録・オープンライセンス対象にしない方針です。
